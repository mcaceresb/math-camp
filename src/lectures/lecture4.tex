...
\begin{example}
  \begin{itemize}[label=$\bullet$]
    \item One thing to note is that the Taylor expansion of any polynomial is eventually the polynomial itself. Consider, for example,
      \[
        f(x) = x^2 + 3x
      \]
      ...
      However, for lower-order Taylor expansions the theorem still applies:
      \begin{align*}
        f(x)
      ...
      In the first case, the expansion is exact for \textit{some} $c$ between $1$ and $x$. In the second case, we have a first-order Taylor approximation. We can visualize both
      \begin{figure}[!ht]
        \centering
        ...
        \begin{tikzpicture}
          ...
          \draw [->, >=latex] (axis cs:0.7, 4.25) -- (axis cs:0.875, 4.25);
            \node[left] at (axis cs:0.7, 4.25) {$c = 0.75 \in (0.5, 1)$};
