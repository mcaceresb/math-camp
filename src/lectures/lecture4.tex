...
\lhead{\color{light-gray} \itshape Math Camp Aug 17, 2023 -- Lecture \thesection}
...
\begin{proof}
  ...
  If we can find $\delta \le \widetilde{\delta}$ s.t. $\left(\varepsilon + |L|\right)\delta < \varepsilon$ then we'd be done.  Take any $\delta < \min\left(\tilde{\delta}, \dfrac{\varepsilon}{\varepsilon + |L|}\right)$ and we have
  \[
...
\begin{theorem}[Taylor's theorem]
  ...
  The first term is called the \keyword{$n$th order Taylor approximation} of $f$ around $\widetilde{x}$ and the second term is called the error or remainder term of the Taylor expansion. If we further have that $f$ is $n + 1$ times continuously differentiable then $\exists c \in (\widetilde{x}, x)$ (or $c \in (x, \widetilde{x})$) s.t.
  \[
    f(x)
    = \sum^{n}_{k = 0} \dfrac{f^{(k)}(\widetilde{x})}{k!} (x - \widetilde{x})^k
    + \dfrac{f^{(n + 1)}(c)}{(n + 1)!} (x - \widetilde{x})^{n + 1}
  \]
  % (i.e. there is an explicit formula for the remainder).
\end{theorem}
...
\begin{proof}
  ...
  (This is subtracting the line with slope $\dfrac{f(b) - f(a)}{b - a}$ that goes through $a$ from the function $f$, which is what we need to get it to ``flatten.'') Note that $g(a) = g(b) = f(b) - f(a)$. Hence $\exists c$ s.t.
  \[
    g^\prime(c) = 0
  \]
...
\begin{proof}
    For Rolle's theorem we will use the extreme value theorem, and we know that $f$ is bounded and attains its $\sup$ and its $\inf$ in $[a, b]$. If the $\sup$ and the $\inf$ are \textit{both} at $\set{a, b}$, then because by assumption $f(a) = f(b)$, $f(x) = 0$ everywhere on $[a, b]$. Take any $c \in (a, b)$, and
    \[
        f^\prime(c)
    ...
    Suppose then that either the $\sup$ or the $\inf$ occur at at interior point $c \in (a, b)$. Take the $\sup$ (WLOG; the $\inf$ is analogous or you can set $g = -f$). By assumption $f$ is differentiable, so $f^\prime(c)$ exists. That is, we know that
    \[
        f^\prime(c)
        = \lim_{y \to c} \dfrac{f(y) - f(c)}{y - c}
        = L
...
(\NB: \textbf{\textit{Stop here in class; the rest is a homework problem.}}) We have that
\begin{align*}
...
\begin{definition}
  A square $N \times N$ matrix $S$ with elements in $\mathbb{R}$ is \keyword{positive semidefinite} (PSD) if $\forall p \in \mathbb{R}^N$
  \[
    p^T S p \ge 0
  \]
and \keyword{positive definite} (PD) if the inequality is strict whenever $p \ne 0$.
\end{definition}

\begin{definition}
  A square $N \times N$ matrix $S$ with elements in $\mathbb{R}$ is \keyword{negative semidefinite} (NSD) if $\forall p \in \mathbb{R}^N$
  \[
    p^T S p \le 0
  \]

  and \keyword{negative definite} (ND) if the inequality is strict $p \ne 0$.
\end{definition}
